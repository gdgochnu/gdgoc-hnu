...
- [ ] 6.2 Build **Broadcast assignment**: when creating a task, let the creator pick `assignment_mode = broadcast` + a target committee; generate a `task_assignees` row per member; each member gets their own independent To Do/In Progress/Submitted status + evidence on their personal task view.
- [ ] 6.3 Build the task owner's **"Review submissions" screen** for a broadcast task: see every member's submission side-by-side, pick/write a consolidated best answer, attach it as the task's own evidence, move the task to `Review`.
