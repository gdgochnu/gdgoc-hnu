...
import { FacultiesSkeleton } from '@/components/skeletons/FacultiesSkeleton';
...
async function FacultiesDataLoader() {
  const admin = createAdminClient();

  // Fetch all faculties sorted by sort_order
  ...
  return (
    <div style={{ padding: '2rem 1.5rem 4rem', maxWidth: '1100px', margin: '0 auto', width: '100%' }}>
      {/* Breadcrumb Header */}
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '1rem', flexWrap: 'wrap', marginBottom: '2.5rem' }}>
        <div>
          ...
          <h1 style={{ fontSize: '2rem', fontWeight: 800, letterSpacing: '-0.02em', marginBottom: '0.5rem', color: '#FFFFFF' }}>
...
export default async function FacultiesSettingsPage() {
  ...
  return (
    <AppShell>
      <Suspense fallback={<FacultiesSkeleton />}>
        <FacultiesDataLoader />
      </Suspense>
    </AppShell>
  );
