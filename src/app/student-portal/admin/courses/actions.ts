...
/**
 * Upload a course cover image directly to Google Drive
 */
export async function uploadCourseCoverImageAction(formData: FormData): Promise<{
  success: boolean;
  url?: string;
  error?: string;
}> {
  try {
    ...
    const cleanFileName = `course_cover_${Date.now()}_${Math.random().toString(36).substring(2, 7)}.${ext}`;
    ...
    console.error('Error in uploadCourseCoverImageAction:', err);
    return { success: false, error: err.message || 'Failed to upload course cover image.' };
  }
}
