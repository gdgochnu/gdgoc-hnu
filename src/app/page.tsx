...
export default async function HomePage(props: HomePageProps) {
  ...
  return (
    <div style={{ position: 'relative', zIndex: 1, minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
          ...
          <Link href="/" className="brand-badge">
            ...
            <div>
              <div className="brand-title">
                GDGoC HNU OS
                <span style={{ fontSize: '0.65rem', padding: '0.15rem 0.5rem', borderRadius: '999px', background: 'rgba(66, 133, 244, 0.15)', color: '#93C5FD', border: '1px solid rgba(66, 133, 244, 0.3)' }}>
                  Active
                </span>
              </div>
              <div className="brand-sub">Helwan National University</div>
            </div>
          </Link>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
            {currentUser ? (
              <UserAuthStatus
                ...
                avatarUrl={userProfile?.avatar_url || currentUser.user_metadata?.avatar_url}
      ...
      {/* Main Content */}
      <main style={{ maxWidth: '1200px', margin: '0 auto', padding: '3.5rem 1.5rem 4rem', flex: 1, width: '100%' }}>
        {/* Error Alert if any */}
        {authError && (
        ...
        <section className="animate-fade-in" style={{ textAlign: 'center', maxWidth: '860px', margin: '0 auto 4.5rem' }}>
          ...
          {/* Action Area based on Auth State */}
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1rem', marginBottom: '2.5rem' }}>
            {currentUser ? (
              <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', justifyContent: 'center' }}>
                {userProfile?.status === 'active' ? (
                  ['president', 'co_president'].includes(userProfile?.role || '') ? (
                    <Link href="/approvals" className="btn-primary" style={{ padding: '0.9rem 2rem', fontSize: '1rem' }}>
                      <span>Leadership Approvals Portal</span>
                      <ArrowRight size={18} />
                    </Link>
                  ) : (
                    <Link href="/onboarding/status" className="btn-primary" style={{ padding: '0.9rem 2rem', fontSize: '1rem' }}>
                      <span>View Membership Status</span>
                      <ArrowRight size={18} />
                    </Link>
                  )
                ) : userProfile?.status === 'pending_review' ? (
                  <Link href="/onboarding/status" className="btn-primary" style={{ padding: '0.9rem 2rem', fontSize: '1rem', background: 'linear-gradient(135deg, #FBBC04 0%, #D97706 100%)' }}>
                    <span>Check Application Status</span>
                    <ArrowRight size={18} />
                  </Link>
                ) : (
                  <Link href="/onboarding/complete-profile" className="btn-primary" style={{ padding: '0.9rem 2rem', fontSize: '1rem' }}>
